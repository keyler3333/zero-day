const BYPASS_METHODS = { DIRECT: 'direct', REWRITE: 'rewrite', XHR: 'xhr' };
    ...
    try {
        ...
        if (method === BYPASS_METHODS.DIRECT) {
            try {
                ...
                if (xhrResponse.ok) return xhrResponse;
            } catch (xhrError) {}
        }
...
function cleanHeaders(headers) {
    ...
    const allowed = ['accept', 'accept-language', 'content-type', 'user-agent', 'cache-control'];
    for (let [key, value] of headers.entries()) {
        if (allowed.includes(key.toLowerCase())) clean.set(key, value);
    }
